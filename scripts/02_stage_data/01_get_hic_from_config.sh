...
## USEGAE bash 01_get_hic_from_config.sh ../genomenotes_pipeline.conf
...
# build map: sample -> hic_dir (col1 -> col3)
...
while IFS= read -r line || [[ -n "$line" ]]; do
  ...
  if [[ -z "$target" ]]; then
    target="/scratch/pawsey0964/${USER_REAL}/genomenotes/${og:-unknown}/hic"
